...
def _sigma_score(value, mean, std, direction="both", min_score=3.5):
    """
    Оценка 3.5–10 по числу σ от нормы.
    direction='both'  — штраф в обе стороны от нормы
    direction='up'    — штраф только за значения НИЖЕ нормы (высокие значения лучше)
    direction='down'  — штраф только за значения ВЫШЕ нормы (низкие значения лучше)
    Шкала: среднее = 10, каждый σ −1.8 балла.
    При 1σ = 8.2, при 2σ = 6.4, при 3σ = 4.6, при 3.6σ+ = 3.5 (пол).
    Красивые → 7–10, средние → 5–7, некрасивые → 3.5–5.
    """
    if std == 0:
        return 10.0
    z = (value - mean) / std
    ...
    if direction == "both":
        ...
        dev = max(0.0, -z)   # штраф только когда ниже нормы
    else:
        dev = max(0.0, z)    # штраф только когда выше нормы
